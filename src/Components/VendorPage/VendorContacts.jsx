...
import Loader from '../Loader_File/Loader'; // ✅ Import your loader here
...
const VendorContacts = ({ contacts, setContacts }) => {
  ...
  const { values, errors, touched, setValues, setTouched } = formik;
  ...
  const validateDefaultContact = (list) => {
    ...
    if (count === 0) {
      setDefaultContactError('At least one contact must be marked as default');
      return false;
    }
    if (count > 1) {
      setDefaultContactError('Only one contact can be default');
      return false;
    }
  ...
  setSavingContactIndex(index); // 🟢 Set loader index here
  ...
  try {
    ...
    setSavingContactIndex(null); // 🔴 Stop loader if validation fails
    return;
  }

  if (!validateDefaultContact(values.contacts)) {
    setSavingContactIndex(null); // 🔴 Stop loader on default validation error
    return;
  }
  ...
  if (vendorId) {
    if (contact.id) {
      await dispatch(putcontactRequest({ ...finalPayload, id: contact.id }));
    } else {
      const match = contacts.find(
        (c) =>
          c.name === contact.name &&
          c.email === contact.email &&
          c.mobileNo === contact.mobileNo &&
          c.isDefault === (contact.isDefault === 'YES')
      );

      if (!match) {
        await dispatch(createContactRequest(finalPayload));
      } else {
        toast.info('This contact is already saved');
      }
    }
  }
  ...
  setValues({ contacts: updated });
  setContacts(updated);
  setSavingContactIndex(null); // ✅ Stop loader after save completes
};
  ...
  const handleDelete = (index) => {
    ...
    validateDefaultContact(updatedList);
  };

  return (
    <FormikProvider value={formik}>
      <div className="contact-section">
        {defaultContactError && hasClickedTick && (
          <div className="default-error-msg">{defaultContactError}</div>
        )}
